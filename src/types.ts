...
/** Cast all object items to the ImmutableCast **/
export type CastObjectToImmutable<T extends object> = immutable.RecordOf<U.Merge<{ [K in A.Keys<T>]: ImmutableCast<A.At<T, K>> }>>;

/** Cast all array items to the ImmutableCast **/
export type CastListToImmutable<T extends any[]> = immutable.List<O.UnionOf<{ [K in A.Keys<T>]: ImmutableCast<A.At<T, K>> }>>;
